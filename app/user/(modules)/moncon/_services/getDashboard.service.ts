...
interface GetDashboardService {
  codeArea: number;
  workType: number;
}

export const getDashboard = async ({ codeArea, workType }: GetDashboardService) => {
  /*  try {
     switch (codeArea) {
       case 'pdmp':
  ...
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/dashboard-stats?work_type=${workType}&area=${codeArea}`);
    console.log('Response from getDashboard service:', response.data);
