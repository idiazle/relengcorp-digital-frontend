...
import { deleteMonconReport, createMonconReport, updateReport, getAllReports } from '@/app/_services/monconServices'
...
/*   const [page, setPage] = useState(1)
  ...
  useEffect(() => {
    if (!openReportModal) {
      getAllReports().then((response) => {
        console.log('Reports fetched:', response.status, response)
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div>
        <p>
          Programados: reportes registrados con anterioridad (rutas) | No Programados: reportes registrados en el momento (sin ruta) | Histórico: reportes eliminados o con fecha de ejecución pasada
        </p>
      </div>
      <div className="flex-1 overflow-auto">
        <Tabs defaultValue="programmed" className="w-full">
          <TabsList className="bg-gray-200 justify-start sticky top-0 z-20 w-full">
            <TabsTrigger value="programmed">{MONCON_UI.OPTIONS.programmed} ({programmedReports.length})</TabsTrigger>
            <TabsTrigger value="not-programmed">{MONCON_UI.OPTIONS.notProgrammed} ({notProgrammedReports.length})</TabsTrigger>
